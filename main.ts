...
function calculaterProduct(a:number, b:number){
    return a*b;
}
...
function sum(val1:string  ,val2:number,val3:number) {        //here val1 and val2 ,val3 are parameters
...
function intro (a:string = "Hello",b:string = "Sir Hamza"){
       return a + " " + b
}
console.log(intro());

// 2.
function sum2 (x:number , y:number = 15){
    return x/y
}
console.log(sum2(3))
...
//  in the array
// Q: 41
let magicianNames : string[] = ["Harry Houdini" , "David Copperfield" ,"Penn & Teller " , "David Blaine", "Criss Angel"," Dynamo"]
...
function show_magicians3( ...magicianNames1: string [] ){
...
function recipe(...sandwich:string []){
...
let fruit: string[] = ["mango", "orange","melon"]
fruit.push("graps")          //add in arry
console.log(fruit)
fruit.pop()                 //remove last  item from arry
fruit.shift()               //remove first item from arry
console.log(fruit);
let  numArr: number[]= fruit.map((fruit) => fruit.length)  
console.log(numArr)


//6. Q: 94
let words: string[] = ["Hello", "World", "TypeScript", "JavaScript"];
let lengthe: number[] = words.map((words) => words.length)
console.log(lengthe)

//7. Q: 95
let num: number[] = [40, 60, 10, 100]
function newnum ( ...num:number[]) {
...
let num3: number[] = [40, 60, 10, 100]
function newnum3 ( ...num3:number[]) {
...
function createExponentiation(base: number) {
...
function contdown(number:number){
    if(number<=0){
        console.log("DONE!")
    return
 }
...
function outerFunction1 (x:number){
...
let anonymouFunction = function (x: number[], callback: (num: number[]) => number[]): void {
    let result = callback(x);
    console.log(result);
}

let doubleFunction = (arr: number[]): number[] => {
...
let info = function(a:string,b:string){
    return a + " " + b
}
